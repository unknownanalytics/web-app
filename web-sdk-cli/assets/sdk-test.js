(function () {
    ...
    class Sender {
      ...
      constructor(config) {
        this.config = config;
        this.debug = config.debug; // WS is not supported
      ...
      _sendXHR(args) {
        ...
        let config = this.config; // : 'bearer ' + config.token
    ...
    class PageViewManager {
      ...
      */
      ...
      constructor(options, sender) {
        ...
        this.sender = sender;
        this.lastLocation = null; // override default function and add custom event change
      ...
      push(body) {
        body = Object.assign({}, body, {
          browser: Utils.getBrowserParams()
        });
        this.sender.push({
          data: body,
          type: VIEW
    ...
    let config = {
      apiURL: 'http://127.0.0.1:3000/collect',
      version: 'beta',
      wsURL: 'PUSH_URL',
      token: null
    };
    ...
    window.unkAnalytics = function (token, options) {
      ...
      PageViewManager.createManager(options, sender); //new EventManager(options, sender);
